...
public class Test {
    public static void main(String[] args) {
        ...
        //1)
//        Map<Integer,List<Student>> gradesMap = al.stream().collect(Collectors.groupingBy(e->e.getGrade()));
//        for(Map.Entry<Integer,List<Student>> entry: gradesMap.entrySet()) {
//            System.out.println(entry.getKey()+": " +entry.getValue());
//        }
        //2)
        Map<Boolean,List<Student>> gradesMap = al.stream().collect(Collectors.partitioningBy(e->e.getAge()>20));
        for(Map.Entry<Boolean,List<Student>> entry: gradesMap.entrySet()) {
            System.out.println(entry.getKey() + ": " + entry.getValue());
        }
